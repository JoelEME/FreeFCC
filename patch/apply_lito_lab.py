#!/usr/bin/env python3
"""Apply the FreeFCC Lito X1 Lab overlay to the pinned upstream FreeFCC checkout.
...
BASE_VERSION = "pinned-3887bc0"
LAB_VERSION = "lito-x1-lab"
...
def main() -> None:
    ...
    if not build.exists():
        fail(f"missing expected file: {build}")
    if 'applicationId = "com.freefcc.litolab"' in build_text:
        fail("Lito X1 Lab appears to be already applied to this checkout")

    copy_overlay(src_root, repo)

    patch_file(build, [
        ('applicationId = "com.freefcc.app"', 'applicationId = "com.freefcc.litolab"', 'applicationId'),
    ])
...
'''
    ...
    print("Variant: Lito X1 Lab (separate applicationId com.freefcc.litolab)")
